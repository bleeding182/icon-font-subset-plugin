...
hb_face_t* perform_subsetting(
    ...
    // Icon fonts don't need bidi mirroring variants (added in HarfBuzz 11.1.0)
    flags |= HB_SUBSET_FLAGS_NO_BIDI_CLOSURE;
    ...
    if (metrics_before.total_size > metrics_after.total_size && metrics_before.total_size > 0) {
    ...
    if (!reduced_tables.empty()) {
        ...
        for (size_t i = 0; i < reduced_tables.size() && i < 5; i++) {
            ...
            int percent = (before > after && before > 0) ? (before - after) * 100 / before : 0;
